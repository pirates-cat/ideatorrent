...
//Create the img  
$image = @imagecreatefrompng("modules/ideatorrent/themes/" . QAPollConfig::getInstance()->getValue("selected_theme") . "/imagelink/img_idea_big.png") or die ("Internal error #1");
...
imagettftext($image, $size, 0, 62, ($ly - $uly) + 20, $white, $verafont, $title);
if(isset($title2))
{
	imagettftext($image, $size, 0, 62, ($ly - $uly)*2 + 3 + 20, $white, $verafont, $title2);
}



//Handle the vote string
$brown = ImageColorAllocate ($image, 65, 33, 6);
$votes = $data->votes . " VOTES, ADD YOURS!";
...
imagettftext($image, $size, 0, 62, 72, $brown, $veraboldfont, $votes);
