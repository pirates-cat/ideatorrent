...
if($models["release"] == null)
{
	if(count($data["newreleaselist"]) > 0)
	{
		...
		$description = 
			t("Here are the implemented !project ideas with no milestone set.",
	...
	else
	{
		...
		$description = 
			t("Here are the implemented !project ideas.",
				array("!project" => (($GLOBALS['gbl_relation'] != null)?"<b>" . $GLOBALS['gbl_relation']->getData()->name . "</b>":
					$this->getThemeSetting("project_name"))));
	}
}
else if($models["most_recent_release"]->getId() == $models["release"]->getId())
{
	$basepath_plus_subtabpath = $basepath;
	$description = 
		t("Here are the implemented !project ideas for the !release release.",
			array("!project" => (($GLOBALS['gbl_relation'] != null)?"<b>" . $GLOBALS['gbl_relation']->getData()->name . "</b>":
					$this->getThemeSetting("project_name")),
...
<?php
	...
	$navbar_options["selected_entry"] = "implemented";
...
<img src="<?php echo "/" . $this->getThemePath(); ?>/images/frontpage-implemented.png" alt="Ideas in development">
